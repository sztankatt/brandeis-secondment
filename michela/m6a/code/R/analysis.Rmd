...
```{r save_candidate_tx_ids, echo = F}
candidate_gene_names <- candidates_diff %>%
	filter(enrichment_diff > 2) %$%
	gene_name

candidate_tx_ids <- txs %>%
	filter(gene_name %in% candidate_gene_names) %$%
	tx_id

dir.create(paste0(wd, 'data/candidates/'))
write_tsv(data.frame(a=candidate_tx_ids), path = paste0(wd, 'data/candidates/m6A_enriched_tx_ids.txt'), col_names = F)
```
