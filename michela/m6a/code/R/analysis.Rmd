...
big_table_enrichment_diff <- enrichment_diff %>%
	...
	arrange(desc(median_enrichment_diff)) %>%
	inner_join(ip_vs_input.resLFC %>%
			   	dplyr::select(gene_name, padj, log2FoldChange), by = 'gene_name') %>%
	dplyr::rename(p_value = padj)

candidate_table <- big_table_enrichment_diff %>%
	filter(median_enrichment_diff > 2, log2FoldChange > 1, p_value < 0.05) %>%
	dplyr::select(-log2FoldChange)

candidate_table %>%
	write_excel_csv(paste0(wd, 'data/candidate_expression.csv'))
```

```{r save_sequences, echo = F}
candidate_ids <- genes %>%
	filter(gene_name %in% candidate_table$gene_name) %$%
	gene_id

rest_ids <- genes %>%
	filter(!(gene_name %in% candidate_table$gene_name)) %>%
	filter(gene_biotype == 'protein_coding', gene_id %in% names(gene_sequences)) %$%
	gene_id
...
candidate_seq <- gene_sequences[candidate_ids]
