...
star_log <- samples %>%
	ldply(function(sample){
		readStarLog(paste0('../../data/mapping/', sample, '.Log.final.out')) %>% data.frame(sample = sample)
	}) %>%
	as_tibble %>%
...
candidates_diff <- candidates %>%
	...
	transmute(gene_name = gene_name,
			  ...
			  selected_enrichment = ifelse(condition == 'IP', min_enrichment, max_enrichment)) %>%
	...
	# filter if enrichment difference between IP and IgG is at least 1.5 higher for the IP
...
candidates_ip_diff <- candidates_ip %>%
	...
	filter(gene_name %in% (candidates_diff %>% filter(enrichment_diff > 0) %$% gene_name))
...
pl_in <- candidates_diff %>%
	filter(enrichment_diff > 0) %>%
	mutate(sig = IP > 2 & enrichment_diff > 1)
...
pl_in %>%
	ggplot(aes(IP, IgG, color = sig)) +
		...
		coord_fixed() +
		theme(legend.position = 'none') +
		labs(x = 'min(m6A/input)', y = 'max(IgG/input)') +
		ggtitle('IP vs IgG enrichment', 'where IP/input > IgG/input')
```
