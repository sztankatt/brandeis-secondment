...
pl_in %>%
	ggplot(aes(median_ip_enrichment, median_igg_enrichment, color = sig)) +
		...
		ggtitle('ip vs igg enrichment', 'ip vs input enriched genes in red')
```
...
m6a_candidate_table <- big_table_enrichment_diff %>%
	filter(median_enrichment_diff > 2, log2FoldChange > 1, p_value < 0.05) %>%
...
writeXStringSet(control_seq,
				paste0(michela_m6a_root, 'data/candidates/control_seqs_2.fa'))
...
m6a_candidate_motifs <- read_table2(paste0(michela_m6a_root, 'data/candidates/fimo_out/fimo.tsv')) %>%
...
motif_coverage <- m6a_candidate_motifs %>%
...
```{r plot_motif_coverage, echo = F, fig.width = 10, fig.height = 5}
all_coverage <- bind_rows(fivep_coverage,
		  cds_coverage,
		  threep_coverage)

all_coverage <- all_coverage %>%
	mutate(bin = ntile(start, 30) - 0.5) %>%
...
coverage_plot <- function(all_coverage, gene_names){
	all_coverage %>%
	...
	ggplot(aes(bin, norm_count, fill = motif_id)) +
		...
		scale_x_continuous(limits = c(-1, 31), breaks = c(0,10, 20, 30), labels = c('5\'', 'cdsS', 'cdsE', '3\''), expand = c(0,0)) +
...
m6a_motif_count <- all_coverage %>%
	group_by(gene_name, feature_type) %>%
	summarise(count = n()) %>%
	spread(feature_type, count, fill = 0)
```

```{r plot_genes_with_many_motifs, echo = F, fig.width = 10, fig.height = 10}
m6a_motif_count %>% 
	filter(cds + threep + fivep >= 40) %$%
	gene_name %>%
	coverage_plot(all_coverage, .) +
		ggtitle('Genes with at least 40 total motifs')

```

```{r plot_genes_with_3p_motif, echo = F, fig.width = 10, fig.height = 8}
...
m6a_3p_candidates %>%
	coverage_plot(all_coverage, .) +
		ggtitle('Genes with most motifs in 3\' UTR')
```

```{r plot_genes_with_5p_motif, echo = F, fig.width = 10, fig.height = 6}
...
```{r plot_genes_with_cds_motif, echo = F, fig.width = 10, fig.height = 6}
...
```{r plot_genes_with_motif_in_both_utrs, echo = F, fig.width = 10, fig.height = 6}
m6a_motif_count %>% 
	filter(cds + fivep + threep > 10) %>%
...
```{r load_find_circ_data, echo = F, include = F}
txdb <- makeTxDbFromGFF(paste0(root_dir, 'shared/annotations/dm6.gtf'))
...
### Abundance of m6a candidates

```{r plot_abundance_of_candidates, echo =F}
abundances$m6a_type = 'not_methylated'

abundances <- abundances %>%
	mutate(m6a_type = ifelse(gene_id %in% m6a_candidate_ids, 'methylated', m6a_type),
		   m6a_type = ifelse(gene_name %in% m6a_5p_candidates, '5p_methylated', m6a_type),
		   m6a_type = ifelse(gene_name %in% m6a_3p_candidates, '3p_methylated', m6a_type),
		   m6a_type = ifelse(gene_name %in% m6a_cds_candidates, 'cds_methylated', m6a_type)) %>%
	filter(m6a_type != 'methylated')

m6a_mu <- abundances %>%
	group_by(m6a_type) %>%
	summarise(grp.mean = mean(mnc.rfp))

abundances %>%
	ggplot(aes(mnc.rfp, color = m6a_type)) +
		geom_density()+
		scale_x_log10() +
		facet_wrap(~m6a_type, ncol = 1) +
		geom_vline(xintercept = m6a_mu %>% filter(m6a_type == 'not_methylated') %$% grp.mean,
				   linetype = 'dashed', color = 'purple') +
		geom_vline(data=m6a_mu, aes(xintercept=grp.mean, color=m6a_type),
             linetype="dashed") +
