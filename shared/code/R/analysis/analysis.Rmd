...
colnames(gene_features) <- c('gene_id', 'tx_id','strand', 'tx_start', 'tx_end',
							 'fivep_start', 'fivep_end', 'threep_start', 'threep_end', 'cds_length', 
							 'exon_id')
...
tx_feature_lengths <- gene_features %>%
	group_by(tx_id) %>%
	summarise(threep = sum(threep_end - threep_start, na.rm = T),
			  fivep = sum(fivep_end - fivep_start, na.rm = T),
			  cds = max(cds_length, na.rm = T))
```

```{r load_sample_specification, echo = F}
config = read_yaml(paste0(root_dir, 'config.yaml'))
m6a_samples <- config$raw_data$michela$m6a$samples %>% names
```

```{r parse_star_logfiles, echo = F}
star_log <- m6a_samples %>%
	ldply(function(sample){
		readStarLog(paste0(michela_m6a_root,'data/mapping/star/', sample, '.Log.final.out')) %>% data.frame(sample = sample)
	}) %>%
	as_tibble %>%
...
esat_counts <- m6a_samples %>%
	...
	# gather back
	gather('sample', 'count', m6a_samples) %>%
	separate(sample, into = c('condition', 'replicate'), extra = 'drop') %>%
...
sample_metadata <- tibble(sample = m6a_samples) %>%
	separate(sample, into = c('condition', 'replicate'), extra = 'drop') %>%
...
txdb <- makeTxDbFromGFF(config$knowledge$annotations$dm6)
dm6_genome <- readDNAStringSet(config$knowledge$genomes$dm6) 
...
gene_exons <- exonsBy(txdb, by = 'gene') %>%
	...
	filter(seqnames %in% c('chr3R', 'chr2R', 'chr3L', 'chr2L', 'chrX', 'chrY', 'chr4')) %>%
...
tx_cds <- cdsBy(txdb, by='tx', use.names = T)
tx_threep <- threeUTRsByTranscript(txdb, use.names = T)
tx_fivep <- fiveUTRsByTranscript(txdb, use.names = T)
```
...
m6a_candidate_table %>%
	...
	write_excel_csv(paste0(michela_m6a_root, 'data/m6a_candidate_expression.csv'))
```
...
abundant_non_target_ids <- enrichment_data %>%
...
abundant_non_target_seq <- gene_sequences[abundant_non_target_ids]

sequence_width_limit <- 2000

### we filter candidate sequences to have similar dist between candidates and control
m6a_candidate_short_seq <- m6a_candidate_seq[width(m6a_candidate_seq) < sequence_width_limit]
m6a_candidate_short_ids <- names(m6a_candidate_short_seq)
```

```{r create_control_seqs, echo = F}
...
# its a semi random approach, where first we identify sequences in an increased window, at least twice as many as a 
	# multiplication factor. Then we chooce as many sequences as high is the multiplication factor. 
get_control_seq <- function(m6a_candidate_seq, reference_seq, sample_size_mult, number_of_iterations = 1){
	# set an iterator for bootstrapping
	iterations <- seq(1, number_of_iterations, 1)
	
	iterations %>% llply(function(iterator){
		m6a_candidate_seq %>% llply(function(sq){
			len <- length(sq)
			
			window_size <- 10
			windowed_seqs <- get_windowed_sequences(reference_seq, len, window_size)
			...
			sample(windowed_seqs, sample_size_mult)
		}) %>%
		unname() %>%
		do.call(c, .) %>%
		unique
...
# get control sequences which are comparable to candidates in size
get_control_seq2 <- function(m6a_candidate_seq, reference_seq, sample_size_mult){
	m6a_candidate_seq %>%
	llply(function(sq){
		len <- length(sq)
		
		selected_windowed_seq_names <- reference_seq %>%
			# create a tibble with only the names and gene widths
			{ tibble(width = width(.), names = names(.))} %>%
			# subtract the length of candidate from the widths of controls
			mutate(width = abs(width - len)) %>%
			# select the closest controls
			top_n(-1*sample_size_mult, width) %$%
			names
		
		reference_seq[selected_windowed_seq_names]
	}) %>%
	unname() %>%
	do.call(c, .) %>%
	unique
...
#control_seq <- get_control_seq2(m6a_candidate_seq, abundant_non_target_seq, 1.5)
control_seq <- get_control_seq(m6a_candidate_seq, abundant_non_target_seq, sample_size_mult = 4, number_of_iterations = 5)
control_ids <- names(control_seq)

control_short_seq <- get_control_seq2(m6a_candidate_short_seq, abundant_non_target_seq, 1)
control_short_ids <- names(control_short_seq) 

rest_ids <- enrichment_data %$%
	gene_id %>%
	unique %>%
	setdiff(m6a_candidate_ids) %>%
	setdiff(names(control_seq)) %>%
	setdiff(abundant_non_target_ids)

# filter out only genes which are in the sequences file
rest_ids <- rest_ids[rest_ids %in% names(gene_sequences)]

rest_seq <- gene_sequences[rest_ids]

detected_seq <- c(m6a_candidate_seq, control_seq, abundant_non_target_seq, rest_seq) %>%
	unique
```

```{r update_tx2gene, echo = F}
# we update the tx2gene table with a type column which specifies if a sequence is a candidate or control
tx2gene <- tx2gene %>%
	mutate(type = 'rest') %>%
	mutate(type = ifelse(gene_id %in% m6a_candidate_ids, 'candidate', type),
		   type = ifelse(gene_id %in% names(control_seq), 'control', type))
```
...
pl_in <- enrichment_diff %>%
	...
	inner_join(genes, by='gene_name') %>%
	mutate(mrna_type = ifelse(gene_id %in% m6a_candidate_ids, 'candidate', 'rest'),
		   mrna_type = ifelse(gene_id %in% names(control_seq), 'control', mrna_type)) %>%
	arrange(desc(mrna_type))

pl_in %>%
	ggplot(aes(median_ip_enrichment, median_igg_enrichment, color = mrna_type)) +
		...
		ggtitle('m6A vs Igg enrichment', 'candidate sequences in red, control sequences in green')

ggsave('figures/tmp.png')
```

```{r save_sequences, echo = F, eval = F}
m6a_candidate_seq_root <- paste0(michela_m6a_root, 'data/sequences/candidates/all/')

writeXStringSet(m6a_candidate_seq,
			paste0(m6a_candidate_seq_root, 'm6a_candidates.fa'))

writeXStringSet(control_seq,
			paste0(m6a_candidate_seq_root, 'control_seqs.fa'))

# write transcript sequences
tx_ids <- tx2gene %>%
	filter(tx_id %in% names(tx_sequences)) %>%
	dplyr::select(tx_id, type)

writeXStringSet(tx_sequences[tx_ids %>% filter(type == 'candidate') %$% tx_id],
				paste0(m6a_candidate_seq_root, 'm6a_candidates_tx.fa'))

writeXStringSet(tx_sequences[tx_ids %>% filter(type == 'control') %$% tx_id],
				paste0(m6a_candidate_seq_root, 'm6a_control_tx.fa'))


```{r save_tx_sequences, echo = F}
# First we need to save all transcript sequences into a single fasta file
# These transcripts will be then shuffled, to identify possible targets and candidates.
...
mrna_tx_ids <- tx2gene %>%
	filter(tx_id %in% names(tx_sequences)) %>%
	filter(gene_id %in% mrna_tx_ids) %$%
	tx_id

mrna_all_tx_seq <- tx_sequences[mrna_tx_ids]

tx_file = paste0(michela_m6a_root, 'data/sequences/shuffled/transcripts.fa')

if(!file.exists(tx_file)){
	writeXStringSet(mrna_all_tx_seq, tx_file)
}
```
...
```{r plot_length_dist, echo = F}
length_dist_data <- tibble(sequence_type = 'rest', width = detected_seq %>% width, gene_id = detected_seq %>% names) %>%
	mutate(sequence_type = ifelse(gene_id %in% abundant_non_target_ids, 'abundant_non_target', sequence_type),
		   sequence_type = ifelse(gene_id %in% names(control_seq), 'control', sequence_type),
		   sequence_type = ifelse(gene_id %in% m6a_candidate_ids, 'candidate', sequence_type))

length_dist_data %>%
	filter(width < 6000) %>%
	ggplot(aes(width, color = sequence_type)) +
		geom_vline(xintercept = 5000, linetype = 2, color = 'grey')+
		geom_vline(xintercept = 1500, linetype = 2, color = 'grey')+
		geom_vline(xintercept = 2000, linetype = 2, color = 'grey')+
		geom_density() +
		labs(x = 'length (nt)', color = '') +
		ggtitle('Candidate sequences are longer') +
		scale_x_continuous(breaks = seq(0, 10000, 1000)) 

ggsave('figures/tmp.png')
```

```{r read_motif_result, echo = F, cache = F}
theme_set(theme_cowplot(12))
source(paste0(root_dir, 'shared/code/R/parseFimoOut.R'))

tx_motif_coverage <- paste0(michela_m6a_root, 'data/fimo_out/all/transcripts.tsv') %>%
	read_table2(col_names = c('motif_id',
							  'motif_alt_id',
							  'tx_id',
							  'start',
							  'stop',
							  ...
							  'score',
							  'p_value',
							  'mathed_sequence'), skip = 1) %>%
	parseFimoOut(tx_feature_lengths, tx2gene, nbins = 15)

# there are some cases when the ncbi and ensembl annotations are not the same, in these cases we might get an 
# overhang in the 5', in which case there will be one more bin. currently this is only conserning 9 genes, so and even then 
# the actual position of the motif (ie cds, 3' utr or 5' utr is not affected). So we just replace 15.5 with 14.5 manually here
tx_motif_coverage <- tx_motif_coverage %>%
	mutate(bin = ifelse(bin == 15.5, 14.5, bin))
```

```{r plot_motif_coverage, fig.width = 10, fig.height = 5, echo = F}
plot_all_motif_coverage <- function(motif_coverage){
	motif_coverage %>%
		group_by(motif_id, bin, type) %>%
		summarise(count = n()) %>%
		ungroup() %>%
		group_by(motif_id) %>%
		mutate(dens = count / sum(count)) %>%
		ggplot(aes(bin, dens, fill = motif_id)) +
			geom_bar(stat = 'identity') +
			scale_x_continuous(breaks = c(0, 5, 10, 15), labels = c('5\'', 'CDS Start', 'CDS End', '3\'')) +
...
ggsave('figures/tmp.png', width = 10, height = 5)
```
...
tx_motif_counts <- tx_motif_coverage %>%
	group_by(tx_id, feature_type) %>%
	summarise(count = n()) %>%
...
tx_motif_counts %>% 
	filter(type == 'candidate',
		   cds + threep + fivep >= 40) %$%
	gene_name %>%
	coverage_plot(tx_motif_coverage, .) +
...
m6a_3p_candidates <- tx_motif_counts %>% 
	filter(type == 'candidate',
		   cds + fivep + threep > 10) %>%
...
m6a_5p_candidates <- tx_motif_counts %>% 
	filter(type == 'candidate',
		   cds + fivep + threep > 10) %>%
...
m6a_cds_candidates <- tx_motif_counts %>% 
	filter(type == 'candidate',
		   cds + fivep + threep > 10) %>%
...
gene2met <- gene2met %>%
mutate(m6a_type = ifelse(gene_id %in% m6a_candidate_ids, 'rest', m6a_type),
...
-->
```{r get_enriched_go_terms, echo = F, eval = F}
m6a_go <- esat_counts %>%
...
-->
```{r plot_candidate_vs_rest_go, echo = F, eval = F}
m6a_go_res %>%
...
txdb <- makeTxDbFromGFF(config$knowledge$annotations$dm6)
annot.file = paste0(root_dir, "dm6.sqlite")
...
circ_experiment_paths = llply(m6a_samples,function(x){
paste0(data.path, '/', x, '/find_circ_run/circ_splice_sites.bed')
})

cdata = data.frame(sample = m6a_samples, filename = circ_experiment_paths %>% unlist)
...
write_excel_csv(polya_circs_table, paste0(michela_m6a_root, 'data/polya_circs_table.csv'))
```
...
fraction_counts <- fraction_fc$counts %>%
as.data.frame %>%
rownames_to_column(var = 'gene_id') %>%
as_tibble %>%
...
```{r load_fimo_results, echo = F}
shuffled_n <- 200

read_fimo_files <- function(files){
	ldply(files, function(fimo_file){
		read_table2(fimo_file) %>%
			filter(startsWith(sequence_name, 'FBtr')) %>%
			group_by(sequence_name, motif_id) %>%
			summarise(motif_freq = n())
	}, .id = 'run_id', .parallel = TRUE) %>%
	as_tibble
}

shuffled_fimo_files <- paste0(root_dir,
							  'michela/m6a/data/fimo_out/all/shuffled_',
							  seq(0, shuffled_n - 1, 1), '.tsv')

all_tx_fimo_file <- c('transcripts_0' = '../../../../michela/m6a/data/fimo_out/all/transcripts.tsv')

names(shuffled_fimo_files) <- paste0('shuffled_', seq(0, shuffled_n - 1, 1))

all_fimo_files <- c(shuffled_fimo_files, all_tx_fimo_file)

all_fimo_occurrences <- read_fimo_files(all_fimo_files)
```

```{r calculate_occurrences, echo = F}
calculate_fimo_occurrences <- function(fimo_occurrences){
	fimo_occurrences %>%
		# run_id for shuffled is shuffled_{n} for normal is transcripts_0 ie only one occurrence
		separate('run_id', into = c('run_type', 'run_number')) %>%
		# we group by run type sequence name and motif id, so that we can select the median from the
		# shuffled sequences
		group_by(run_type, sequence_name, motif_id) %>%
		summarise(median_freq = median(motif_freq)) %>%
		spread(run_type, median_freq, fill = 0) %>%
		# rename the median_shuffled and transcript frequency rows
		dplyr::rename(median_shuffled_freq = shuffled,
					  normal_freq = transcripts)
}

joint_occurrences <- calculate_fimo_occurrences(all_fimo_occurrences)
```

```{r calculate_log_odd_ranks, echo = F}
calculate_log_odds <- function(fimo_occurrences){
	fimo_occurrences %>%
	#	group_by(sequence_name) %>%
	#	summarise(median_shuffled_freq = sum(median_shuffled_freq),
	#			  normal_freq = sum(normal_freq)) %>%
		# calculate the probability of seeing a motif not just by chance
		mutate(p = normal_freq / (normal_freq + median_shuffled_freq)) %>%
		dplyr::rename(tx_id = sequence_name) %>%
		# NOTE: here join command removes the transcripts for which no occurrence was found either
		# in the normal sequence, nor the shuffled one.
		inner_join(tx2gene, by = 'tx_id') %>%
...
log_odds <- calculate_log_odds(joint_occurrences)

log_odds$sequence_type = 'rest'

log_odds <- log_odds %>%
	mutate(sequence_type = ifelse(gene_id %in% names(m6a_candidate_seq), 'candidate', sequence_type),
		   sequence_type = ifelse(gene_id %in% names(control_seq), 'control', sequence_type))
```

```{r plot_logodds_density, echo = F}
log_odds %>%
	ggplot(aes(log_odds, color = sequence_type)) +
		geom_density() +
		facet_wrap(~motif_id, ncol = 2) +
		ggtitle('Density of log-odds rate of different sequence types', subtitle = 'bootstrapped di-nucleotide shuffles used as background model')

ggsave('figures/tmp.png', height = 7, width = 11)
```

```{r calc_roc, echo = F, fig.height=7.5, fig.width=7.5}
calculate_roc <- function(log_odds){
	log_odds %>%
		group_by(motif_id) %>%
		filter(sequence_type != 'rest', sequence_type != 'novel_candidates') %>%
		arrange(desc(log_odds)) %>%
		mutate(tp = cumsum(sequence_type == 'candidate'),
			   fp = cumsum(sequence_type == 'control')) %>%
		# reverse the log odds to find out the negative rates
		arrange(log_odds) %>%
		mutate(tn = cumsum(sequence_type == 'control'),
			   fn = cumsum(sequence_type == 'candidate')) %>%
		mutate(p = sum(sequence_type == 'candidate'),
			   neg = sum(sequence_type == 'control'),
		# calculate the rates
				tpr = tp / p,
				tnr = tn / neg,
				fpr = fp / neg
...
roc <- calculate_roc(log_odds) %>%
	dplyr::rename(model_type = motif_id)

log_odds_cutoff <- -0.3
auc <- roc %$% tpr %>% mean
```

```{r check_roc_when_ordering_by_length, echo =F, fig.width = 7.5, fig.height = 7.5}
roc_length <- length_dist_data %>%
	filter(sequence_type == 'candidate' | sequence_type == 'control') %>%
	arrange(desc(width)) %>%
	mutate(tp = cumsum(sequence_type == 'candidate'),
		   fp = cumsum(sequence_type == 'control')) %>%
	# reverse the log odds to find out the negative rates
	...
	arrange(desc(width)) %>%
	mutate(model_type = 'length')
```

```{r plot_roc_function, echo = F}
plot_roc <- function(roc, grouping_variable = NA){
	if(is.na(grouping_variable)){
		gpl <- roc %>% ggplot(aes(fpr, tpr))
	} else {
		gpl <- roc %>% ggplot(aes_string('fpr', 'tpr', color = grouping_variable)) +
			scale_color_manual(values = c(wes_palette(n=5, name='Zissou1'), 'black'))
	}
	
	gpl +
		geom_step() +
		geom_abline(slope = 1, intercept = 0, color = 'grey', alpha = 0.8) +
		#geom_label_repel(data=roc %>%
		#					filter(log_odds > log_odds_cutoff) %>%
		#					tail(1),
		#				 aes(label=paste("log-odds threshold: ", log_odds_cutoff, "\n",
		#								 "sensitivity:", round(tpr, 3), " (", tp, "TP out of", p, "P)\n",
		#								 "specificity:", round(1-fpr, 3), " (", fp, "FP out of", neg, "N)")),
		#				 ylim = c(0.87, NA)) +
		#geom_point(data = roc %>% filter(log_odds > log_odds_cutoff) %>% tail(1), color = 'red', size = 2) +
		#annotate(geom='text', x = 0.25, y = 0.5, label = paste0("AUC=", round(auc, 3))) +
		scale_y_continuous(expand = c(0,0)) +
		scale_x_continuous(expand = c(0,0)) +
		ggtitle('ROC curve of log-odds score for m6A target prediction') +
		coord_fixed()
}

bind_rows(roc, roc_length) %>%
	plot_roc(grouping_variable = 'model_type')

ggsave('figures/tmp.png')
```

Using a log-odds cutoff of 0.1 we achieve almost 90% specificity, with 59% sensitivity. This means that with this cutoff, 9/10 molecules will be indeed methylated, even if we don't identify all the methylated molecules.

If we use this cutoff genome-wide, we identify 2979 novel methylation candidates. There are several reasons why these might not have ended up in our candidate set. First of all, methylation might differ accross tissues and cell-types, so it might be that these novel candidates are simply not methylated in fly-heads.

#### Novel methylation candidates

```{r plot_novel_candidate_abundance, echo = F, fig.width = 9, fig.height = 4}
log_odds <- log_odds %>%
	mutate(sequence_type = replace(sequence_type, log_odds > log_odds_cutoff & sequence_type == 'rest', 'novel_candidates'))

n_novel_candidates <- log_odds %>% 
	filter(sequence_type == 'novel_candidates') %>%
	nrow

joined_count_data <- esat_counts %>%
	left_join(log_odds, by = 'gene_id') %>%
	mutate(sequence_type = ifelse(is.na(sequence_type), 'rest', sequence_type))

joined_count_data %>%
	group_by(gene_id, condition, sequence_type) %>%
	summarise(tpm = median(tpm)) %>%
	ggplot(aes(sequence_type,tpm, fill = sequence_type)) +
		...
		labs(fill = '') +
		ggtitle('Novel candidates are less abundant are less abundant',
				subtitle = paste0('log_odds > ', log_odds_cutoff, ', n = ', n_novel_candidates)) 

ggsave('figures/tmp.png', height = 6, width = 12)
```

```{r assign_sequence_type2_to_log_odds, echo =F}
log_odds <- log_odds %>%
	mutate(sequence_type2 = sequence_type,
		   sequence_type2 = ifelse(sequence_type == 'candidate' & log_odds > log_odds_cutoff, 'true_positive', sequence_type2),
		   sequence_type2 = ifelse(sequence_type == 'candidate' & log_odds <= log_odds_cutoff, 'false_negative', sequence_type2),
		   sequence_type2 = ifelse(sequence_type == 'control' & log_odds > log_odds_cutoff, 'false_positive', sequence_type2),
		   sequence_type2 = ifelse(sequence_type == 'control' & log_odds <= log_odds_cutoff, 'true_negative', sequence_type2)) %>%
	right_join(length_dist_data %>% dplyr::select(-sequence_type), 'gene_id') %>%
	mutate(sequence_type = ifelse(is.na(sequence_type), 'rest', sequence_type)) %>%
	mutate(sequence_type2 = ifelse(is.na(sequence_type2), 'rest', sequence_type2))
```

```{r plot_sequence_type_by_length, echo = F}
log_odds_n <- log_odds %>%
	group_by(sequence_type2) %>%
	summarise(count = n())

log_odds %>%
	ggplot(aes(sequence_type2, width, fill = sequence_type)) +
		scale_fill_manual(values = wes_palette('BottleRocket2')) +
		scale_x_discrete(labels = c('TP', 'FN', 'TN', 'FP', 'novel_c', 'rest'),
						 limits = c('true_positive', 'false_negative', 'true_negative', 'false_positive', 'novel_candidates', 'rest')) +
		geom_boxplot() +
		annotation_logticks(sides = 'l') +
		labs(x = '', y = 'length (nt)', fill = '') +
		annotate(geom = 'text', y = 100000, x = log_odds_n$sequence_type2, label = paste0('n=',log_odds_n$count)) +
		scale_y_log10(limits = c(100, 100000), breaks = c(300, 1000, 3000, 10000, 30000)) +
		ggtitle('Motif based model captures long transcripts more likely')
...
```{r save_shorter_sequences, echo = F, eval = F}
short_seq_root <- paste0(michela_m6a_root, 'data/sequences/candidates/short/') # 

writeXStringSet(m6a_candidate_short_seq,
			paste0(short_seq_root, 'm6a_candidates.fa'))

writeXStringSet(control_short_seq,
			paste0(short_seq_root, 'control_seqs.fa'))
```

## circRNA localisation
...
fraction_circ_abundance <- fraction_circs %>%
	inner_join(fraction_reads, by = c('condition', 'replicate')) %>%
	mutate(norm_count = value / input_reads * 1e6)

fraction_circ_abundance <- fraction_circ_abundance %>%
	group_by(chr, start, end, strand, condition) %>%
	summarise(mnc = mean(norm_count)) %>%
	filter(condition %in% c('homog', 'pel1', 'pel2', 'sup2')) %>%
	spread(condition, mnc) %>%
	dplyr::rename(nuclei = pel1,
				  synapses = pel2,
				  cytoplasm = sup2) %>%
	gather('condition', 'abundance', homog:cytoplasm)

fraction_circ_abundance %>%
	ggplot(aes(abundance, color = condition)) +
...
pl_in <- fraction_circ_abundance %>%
	group_by(chr, start, end, strand, condition) %>%
	summarise(mna = mean(abundance)) %>%
	spread(condition, mna)

plot_circ_abundance <- function(abundance, x, y){
	ticks <- c(0.01, 0.1, 1, 10, 100)

	abundance %>%
		ggplot(aes_string(x, y)) +
			geom_point(size = 0.7, alpha = 0.8) +
			scale_x_log10(breaks = ticks, labels = ticks, limits = c(0.01, 100)) +
			scale_y_log10(breaks = ticks, labels = ticks, limits = c(0.01, 100)) +
			coord_fixed()
} 

pl_in %>% plot_circ_abundance('synapses', 'cytoplasm') 
ggsave(filename = 'figures/tmp.png')
```
